...
export const useCreateChildProfile = () => {
    return useMutation({
        mutationFn: async (data: CreateChildPayload): Promise<Child> => {
            const response = await axiosInstance.post("/User/CreateChildProfile", data);
            return response.data;
        },
        onSuccess: () => {
            toast({
...
export const useGetChildProfile = (childId: string) => {
    return useQuery<Child>({
        ...
        queryFn: async (): Promise<Child> => {
            const response = await axiosInstance.get(`/User/GetChildProfile/${childId}`);
            return response.data;
        },
        onError: handleError,
    });
...
export const useGetTransactionHistory = () => {
    return useQuery<TransactionDetails[]>({
        ...
        queryFn: async (): Promise<TransactionDetails[]> => {
            const response = await axiosInstance.get("/User/GetTransactionHistory");
            return response.data;
        },
        onError: handleError,
    });
