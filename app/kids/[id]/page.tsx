...
import {useGetChildProfile, useUpdateChildProfile, useDeleteChildProfile, useUserData} from "@/queries/user";
...
import { SearchClassResult } from "@/app/class/Class.types";
...
const EditChildPage = () => {
    ...
    const { data: classes, isLoading: loadingClasses } = useSearchClasses(enteredClassName);
    ...
    const handleUpdate = async (data: EditKidFormValues) => {
        await updateChildProfile.mutateAsync({
            ...
            data: { name: `${data.firstName} ${data.lastName}`, classId: data.classId },
        });
        router.push("/kids");
    };
